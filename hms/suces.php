...
$clientId = 'your_client_id'; // Replace with your actual PayPal client ID
$clientSecret = 'your_client_secret'; // Replace with your actual PayPal client secret
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['paypal'])) {
    ...
    $request->body = [
        ...
        "application_context" => [
            ...
            "return_url" => "http://localhost/dentacare/hms/book-appointment.php?success=true&token={token}"
        ]
    ];
...
if (isset($_POST['submit'])) {
    ...
    if (!preg_match("/^\d{4}-\d{2}-\d{2}$/", $appdate)) {
        $_SESSION['msg'] = 'Invalid date format. Please use YYYY-MM-DD.';
        $_SESSION['msg_type'] = 'error';
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    }
